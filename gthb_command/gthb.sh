...
function github {
    ...
    if [[ "$1" == "$var_move" ]]; then														###############
		if [[ "$2" == "$var_42_git" ]]; then												#
			if [[ $3 ]]; then																#
				cd /home/$var_usr/Documents/Cursus_42/$3									#
			elif [[ !$3 ]]; then															#
			...
			if [[ "$3" == "$var_perso" ]]; then												#
            	cd /home/$var_usr/Downloads/.42_github/Cursus_42							#
        	elif [[ "$3" == "$var_libft" ]]; then											#
            	cd /home/$var_usr/Downloads/.42_github/Cursus_42/Libft						#	Github
        ...
        if [[ "$2" == "$var_web_github" ]]; then											#
            ...
            if [[ "$3" == "$var_project_piscine" ||  "$3" == "$var_project_mc" ]]; then		#
                ...
                git clone "$var_web_link/Stud.git" Cursus_42								#
				cd Cursus_42																#
				echo -e "\033[32mYour project was clone in the corresponding folder\033[0m"	#
			elif [[ "$3" == "-rm" ]]; then													#
				rm -rf !(My_commands)														#
